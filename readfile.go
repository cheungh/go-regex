...
func findRegexInFile(path string, listResult *list.List, compiledPattern *list.List) {
	...
	for w := 1; w <= 3; w++ {
		wg.Add(1)
		go matchPattern(jobs, results, wg, compiledPattern)
	}
...
func matchPattern(jobs <-chan string, results chan<- resultStruct, wg *sync.WaitGroup, pattern *list.List) {
